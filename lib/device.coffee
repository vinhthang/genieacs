  ...
  get: (patterns, callback) ->
    ...
    f = (parameter, properties, flags, children) =>
      ...
      if children is null # This is a rightmost parameter
        ...
        if not (properties[0]? or properties[1]?)
          return res
      ...
      if not (flags[0] > 1 or flags[1] > 1)
        ...
        if (wildcard = patternArray.indexOf('*', parameterArray.length)) == -1
          res.fetch[parameterArray.concat(patternArray[parameterArray.length...]).join('.')] |= 1
        else
          res.fetch[parameterArray.concat(patternArray[parameterArray.length...wildcard]).join('.')] |= 2
    ...
    for pattern in _patterns
      patternArray = if pattern == '' then [] else pattern.split('.')
    ...
    @fetch((err) =>
      ...
      for pattern in unsatisfied
        ...
        # TODO Assert that res.fetch is empty
